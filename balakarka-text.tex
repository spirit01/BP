...
\thesissetup{
    ...
    titleEn       = A~combined EHT/DFT study of Si and P coordination environment influence on the creation of SiO$_6$ centers,
    TeXtitleEn    = A~combined EHT/DFT study of Si and P coordination environment influence on the creation of SiO$_6$ centers,
    keywords      = {silikofosfáty; orbitaly; HSAB; EHT; DFT},
    TeXkeywords   = {silikofosfáty; orbitaly; HSAB; EHT; DFT},
    keywordsEn    = {silicophosphates; orbitals; HSAB; EHT; DFT},
    TeXkeywordsEn = {silicophosphates; orbitals; HSAB; EHT; DFT},
}
\thesislong{abstract}{
Práce je zaměřena na teoretické studium jednotek \ce{SiO4} a~\ce{SiO6} v~silikofosfátových polymerech. Třináct modelových struktur, v rozpětí od  kyseliny orthokřemičité, \ce{H4SiO4}, po komplex \ce{(Si^{VI}(PO4)6(Si^{IV}O4Et2)6)^{2–}}, bylo studováno na úrovni rozšířené Hückleovy metody a~teorie funkcionálu hustoty. Hlavním faktorem, který je sledován je energetická vzdálenost orbitalů HOMO a~LUMO, kde LUMO je nejnižší neobsazený orbital. Z~tohoto parametru lze podle teorie HSAB určit tvrdost/měkkost kyseliny nebo báze a~následnou reaktivitu s příslušným acidobazickým partnerem. Z~experimentálních pozorování vyplývá, že křemík ztrácí schopnost koordinovat se do čísla šest, pokud je na něj přímo navázán uhlík, z tohoto důvodu je mezi studovanými sloučeninami zahrnuta také molekula kyseliny orthokřemičité s jednou OH skupinou substituovanou methylem. \\
  Na tento problém práce  pohlíží jednak ze strany nestability příslušného  systému s šestikoordinovaným křemíkem nesoucím methyl, jednak ze strany HSAB parametrů čtyřkoordinovaného křemíku nesoucího methyl jako prekurzoru. 
Jako nejjednodušší pár modelových sloučenin byly zvoleny kyselina orthokřemičitá \ce{H4SiO4} a její derivát \ce{H3SiO3CH3}. Naše výpočty ukázaly, že substituce methylem snižuje tvrdost této kyseliny o 0,4 eV, což může vysvětlovat její sníženou ochotu reagovat s molekulami fosfátu, jež jsou tvrdými bázemi.  \\
Pro upravenou krystalovou strukturu jsou obměňovány okolní ligandy a~fosfor je nahrazen uhlíkem. Pro takto upravené struktury je opět sledován parametr HSAB a energie orbitalů HOMO a LUMO. Na závěr je porovnána námi upravená krystalová struktura s~experimentálními hodnotami. Kritériem je absolutní chemické stínění šestikoordinovaného křemíku. 
}
\thesislong{abstractEn}{
The thesis focuses on a theoretical study of \ce{SiO4} and \ce{SiO6} units in the
sillicophosphates polymers. 13 molecular structures have been studied;
ranging from silicic acid, up to the complex \ce{(Si^{VI}(PO4)6(Si^{IV}O4Et2)6)^{2–}} on the
level of extended Hückle method and density functional theory. Main
focus which is observed is energy distance of HOMO and LUMO orbitals,
where LUMO is the lowest unoccupied orbital. From this parameter,
it is possible to determine hardness/softness of an acid or base
according to the HSAB theory and the following reactivity with
corresponding acidobazic copartner. From experimental observings emerge,
that silicon looses its ability to coordinate itself up to the number
six, provided there is carbon bonded directly to it. Therefore, a
molecule of silicic acid with one OH group substituted with
methyl is also included in the studied set of compounds.

This matter is in the thesis approached both from the side of the
instability of the corresponding system with six-coordinated silicon
with methyl, and  from the side of HSAB parameters four-coordinated
silicon with methyl as a precursor. Silicic acid and
its derivate \ce{H3SiO3CH3} were selected as the simplest pair of compounds.

Our calculations show, that the substitution with methyl decrease
hardness of this acid by 0,4 eV, which can explain it's decreased
willingness to react with molecules of phosphate, which are hard
bases. For altered crystal structure are being exchanged surrounding
ligands and phosphorus is replaced with carbon. For such altered
structures is again monitored parameter HSAB and energy of HOMO and LUMO
orbitals. With the absolute chemical shift of six coordinated
silicon as a measure, we compare our modified crystal structure with
experimental results.
     
}
\thesislong{thanks}{
    Na tomto místě bych chtěla v~první řadě poděkovat doc. Mgr. Markétě Munzarové, Dr. rer. nat. za neskutečnou ochotu, trpělivost a~cenné připomínky při psaní této bakalářské práce, a~to až do jejího konce. Dále bych chtěla poděkovat svému příteli Lukáši Němcovi za obrovskou trpělivost, péči, podporu během psaní a v neposlední řadě za pomoc s~řešením technických nedostatků a to nejen s LaTeXem. Poděkovat bych chtěla také Tomáši Račkovi za pomoc s typografickou a jazykovou korekturou. Neméně také za cenné rady ohledně MetaCentra. Nesmím zapomenout ani na své přátele a rodinu, protože mi drželi palce až do úplného konce. \\
    Na závěr bych chtěla poděkovat MetaCentru za umožnění kvantově-chemických výpočtů.
...
\begin{figure}
\begin{center}
\subfigure[Prostředí kolem šestikoordinovaného Si, první pozorovaný typ koordinace \cite{Styskalik2015thesis}.]{\includegraphics[width=8cm]{si_koordinovane_6_P.png} \label{img1}}

\subfigure[Prostředí kolem šestikoordinovaného Si, druhý pozorovaný typ koordinace \cite{Styskalik2015thesis}.]{\includegraphics[width=8cm]{si_koordinovany_6_C.png}\label{img2}}
...
\end{equation}
 ...
 V~současné době existuje obrovské množství sad bázových funkcí, které lépe či hůře popisuje zvolený problém. Mezi základní báze jsou uváděny DZP, STO-3G a~různé druhy 6-31G. DZP je double-zeta gaussovská báze s~polarizací, STO-3G jsou Slaterovy orbitaly zjednodušeny jako lineární kombinace tří primitivních gaussiánů, odpovídá minimální bázi. 3-21G jsou označovány jako \uv{split valence basics sets}. Vnitřní elektrony jsou popisovány třemi primitivními gaussiány, valenční orbitaly jsou rozděleny do dvou primitivních gaussiánů a~jednoho jednoduchého gaussiánu. Pro bázi 6-31G je vnitřní oblast tvořena šesti primitivními gaussiány, ostatní je obdobné jako pro 3-21G. Báze 6-21*G je v~základu obdobná, symbol * označuje polarizační funkci. Přidává $p$ orbital pro vodíky a~$d$ orbitaly pro těžší atomy. Symbol ** označuje přidání $d$ orbitalů pro vodík \cite{lowe2011quantum}. \\
...
\caption{Interakce $\bra{22}{\hat{H}}\ket{26}$, $\bra{7}{\hat{H}}\ket{26}$ z~tabulky \ref{tab_h4sio4_vysledky} pro \ce{H4SiO4}.}
...
\caption{Interakce $\bra{11}{\hat{H}}\ket{21}$, $\bra{19}{\hat{H}}\ket{21}$ z~tabulky \ref{tab_h4sio4_vysledky} pro \ce{H4SiO4}.}
...
\caption{Interakce $\bra{22}{\hat{H}}\ket{26}$, $\bra{7}{\hat{H}}\ket{26}$  z~tabulky \ref{tab_sioh3ch3_vysledky} pro \ce{H3SiO3CH3}.} 
...
\caption{Interakce $\bra{7}{\hat{H}}\ket{25}$ z~tabulky \ref{tab_sioh3ch3_vysledky} pro \ce{H3SiO3CH3}.}
...
\caption{Interakce $\bra{21}{\hat{H}}\ket{23}$  z~tabulky \ref{tab_sioh3ch3_vysledky} pro \ce{H3SiO3CH3}.}
...
\caption{Interakce $\bra{30}{\hat{H}}\ket{34}$, $\bra{24}{\hat{H}}\ket{34}$  z~tabulky \ref{tab_h6sio6_vysledky} pro  \ce{(H6SiO6)^{2-}}.}
...
\caption{Interakce $\bra{21}{\hat{H}}\ket{32}$, $\bra{28}{\hat{H}}\ket{32}$ z~tabulky \ref{tab_h6sio6_vysledky}  pro  \ce{(H6SiO6)^{2-}}.}
...
\caption{Interakce $\bra{22}{\hat{H}}\ket{33}$ z~tabulky \ref{tab_h6sio6_vysledky}  pro  \ce{(H6SiO6)^{2-}}.}

\label{obr_h6sio6_vysledky_III}\end{center}
\end{figure} 
%----------------------------------------------------------------------------------------
 \subsection{Molekula H$_3$SiO$_4$(H$_2$PO$_3$)}
 Pro molekulu \ce{H3SiO4(H2PO3)} na obrázku \ref{obr_h3sio4_h2po3} byla nutná oprava zvolených fragmentů. Molekula postrádá symetrii, která by usnadňovala výpočet. Jako fragment jedna byla zvolena část \ce{Si(OH)3} a~fragment dva ligand \ce{O(H2PO3)}. Tato úprava snížila výpočetní náročnost a~bylo možné získat informace o~FMO. Na pravé straně interakčního diagramu je vidět fragment křemíku s třemi OH skupinami, na levé pravé fragment tvořeny zbytkem molekuly. Interakční orbital na obrázku \ref{h3sio4_h2po3_diagram_upraveny} poskytl informace o FMO a jejich příspěvku do překryvové populace. Fragmentové orbitaly  7, 15, 19, 22 a~29 se navzájem mísí za vzniku MO číslo 4, 14, 30, 35 a~41 znázorněných na obrázku \ref{obr_sio3p_vysledky_I}.   

\begin{figure}
\begin{center}
\subfigure[Interakční diagram pro \ce{H3SiO4(H2PO3)}.]{\includegraphics[width=10cm]{h3sio3P_diagram.eps} \label{h3sio4_h2po3_diagram_upraveny}}

\subfigure[Optimalizovaná struktura \ce{H3SiO4(H2PO3)}.]{\includegraphics[width=10cm]{obr_h3sio4_h2po3.png}\label{obr_h3sio4_h2po3}}
...
\subfigure[\ce{(Si^{VI}(PO4)6(Si^{IV}O4Et2)6)^{2-}} ]{\includegraphics[width=10cm]{obr_si_kryst_struktura_real.png} 
...
Krystalová struktura byla pro výpočet absolutního chemického stínění upravena, přesto byla dobrá shoda s~experimentem. Do budoucna by bylo zajímavé rozebrat i další modifikace silikofosfátů a~porovnat stabilitu vůči množství uhlíků vázaných na centrální křemík. Struktura \ce{(Si(PO4)4(COOCH3)2(Si(OH)2)6)^{4-}} byla z důvodu vysokého náboje nestabilní. Do budoucna by bylo vhodnější použít kyslíky nasycené protony, eliminovalo by se tím riziko vzniku umělých vodíkových vazeb.
  
  \chapter{Výpočetní detaily}
  Struktury všech molekul v této práci byly optimalizovány metodou B3LYP v bázi 6-31G* v implementaci programu Gaussian09 \cite{g09}. Molekula \ce{(Si(PO4)6(Si(OH)2)6)^{2-}} byla navíc optimalizována i v bázi IGLO-III \cite{EMSL} za účelem získání přesnější struktury pro NMR výpočet. Na centrální molekulu křemíku a~nejbližší okolí byla použitá báze IGLO-III, na vzdálené okolí báze 6-31G*. Struktury byly nejprve navrženy programem Avogadro \cite{Avogadro}, zobrazení výstupu z~Gaussianu  bylo prováděno programem MOLDEN \cite{Molden}. Programem MOLDEN byly vytvořeny konečné obrázky struktur. Vybrané struktury byly použity jako vstup pro program C.A.C.A.O. \cite{cacao}. Z~interakčního diagramu a~analýzou FMO byly vybrány MO, které byly zobrazovány grafickým prostředím C.A.C.A.O. Stejný program byl použit pro tvorbu interakčních diagramů. \\
Vstupní soubory pro optimalizaci programem Gaussian09 jsou uvedeny jako příloha A. Krystalová struktura \ce{(Si^{VI}(PO4)6(Si^{IV}O4Et2)6)^{2-}} byla stažena z krystalografické databáze na webu \cite{CCDC}, zobrazena programem Mercury \cite{Mercury}, zbytky rozpouštědla byly odstraněny v~programu Avogadro. Následně byl proveden výpočet optimalizace programem Gaussian09.
...
H         -2.82697       -3.66462       -3.18196
 \end{lstlisting}
 \begin{lstlisting}[frame=single, caption={\ce{Si^{VI}(PO4)6(Si^{IV}O4Et2)6^{2-}}},label=DescriptiveLabel]
OPT KRYST.STRUKTURA
...
H        4.32121        -4.24020         6.55472
 \end{lstlisting}
  ...
  \begin{lstlisting}[frame=single, caption={\ce{(Si(PO4)4(COOCH3)2(Si(OH)2)6)^{4-}}, náhrada C za P v~rámci jednoho kuhu},label=DescriptiveLabel]
OPT 
